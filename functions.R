...
# to set up functions needed for the first Food and Brain Study
...
rmcorr.matrix=function(var_names, idvar, data, nreps = 100){
  l=length(var_names)
  res_matrix=matrix(numeric(0),l,l)
  dimnames(res_matrix) <- list(var_names, var_names)
  icount=0
  ...
  for (i in 1:l) {
    ...
    for (j in 1:l){
      ...
      c_res=round(rmcorr(idvar, x, y, data, CI.level = 0.95, CIs = 'bootstrap', nreps = nreps, bstrap.out = F)$r, 3)
